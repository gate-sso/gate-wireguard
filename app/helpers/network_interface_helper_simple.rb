...
module NetworkInterfaceHelper
  def self.get_default_gateway_interface
    begin
      # Simple approach: get default route and extract interface
      default_route = `ip route | grep default 2>/dev/null | head -1`.strip

      if default_route.empty?
        # Fallback: get first non-loopback interface from ip addr
        return get_first_interface
      end

      # Extract interface name from default route
      # Format: "default via 192.168.1.1 dev eth0 proto dhcp metric 100"
      interface_match = default_route.match(/dev\s+(\w+)/)
      ...
      # Extract IP address: "inet 192.168.1.100/24 brd 192.168.1.255 scope global dynamic eth0"
  ...
  class << self
    ...
    def get_first_interface
      # Get first non-loopback interface with IP address
      interface_output = `ip addr show 2>/dev/null | grep -E '^[0-9]+:|inet ' | grep -v '127.0.0.1'`
      ...
      interface_output.each_line do |line|
        ...
        if line.match(/^(\d+):\s+(\w+):/)
          current_interface = $2
          next if current_interface == 'lo' # Skip loopback
        elsif line.match(/inet\s+([0-9.]+)/) && current_interface
          return {
