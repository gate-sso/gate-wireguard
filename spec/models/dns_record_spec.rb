...
RSpec.describe DnsRecord, type: :model do
  ...
  describe '.dns_record_exists?' do
    ...
    it 'should add the dns record to redis' do
      ...
      result = DnsRecordsHelper.get_ip_addres('orbit.soracloud.net', dns_server: @dns_server,
                                                                     dns_server_port: @dns_server_port)
      expect(result.to_s).to eq('10.8.1.1')

      result = DnsRecordsHelper.get_ip_addres('orbit01.soracloud.net', dns_server: @dns_server,
                                                                       dns_server_port: @dns_server_port)
