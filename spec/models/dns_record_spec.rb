...
RSpec.describe DnsRecord do
  ...
  describe '.dns_record_exists?' do
    ...
    it 'adds the dns record to redis' do
      ...
      allow(DnsRecordsHelper).to receive(:get_ip_addres).with("orbit.#{@dns_zone}", dns_server: @dns_server, dns_server_port: @dns_server_port).and_return('10.8.1.1')
      allow(DnsRecordsHelper).to receive(:get_ip_addres).with("orbit01.#{@dns_zone}", dns_server: @dns_server, dns_server_port: @dns_server_port).and_return('10.8.2.2')
